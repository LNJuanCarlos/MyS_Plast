...
export class FormproductoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.producto.id_PRODUCTO != null) {
      this.actualizarCamposMarca();
      this.originalProducto = JSON.parse(JSON.stringify(this.producto));
    }
  }

  isModified(): boolean {
    return JSON.stringify(this.originalProducto) !== JSON.stringify(this.producto);
  }
  ...
  cerrarModal() {
    ...
    this.productocom.actualizarListadoProductos(this.producto);
  ...
  cerrarModalSinCambios() {
    if (this.isModified()) {
      Swal.fire({
        title: "FORMULARIO DE PRODUCTO",
        text: "Hay cambios sin guardar en el formulario, está seguro que desea salir?",
        icon: 'info',
        ...
        if (result.isConfirmed) {
          ...
          this.modalservice.cerrarModal();
          this.productocom.actualizarListadoProductos(this.originalProducto);
